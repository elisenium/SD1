...
public class ListeDoubleMixte {
	...
	public boolean auMoins1Belge(){
		...
		while (baladeur != null && baladeur != queue) {
			if (baladeur.joueur.getNationalite().equals("B"))
				return true;
	...
	public int nombreNationalitesDifferentes(){
		...
		HashSet<String> ensembleNationalites = new HashSet<>();

		Noeud baladeur = tete.suivant;
		while (baladeur != null && baladeur != queue) {
			ensembleNationalites.add(baladeur.joueur.getNationalite());
			baladeur = baladeur.suivant;
		}
		return ensembleNationalites.size();
	...
	public boolean supprimerCouple (JoueurTennis joueur) {
		...
		Noeud noeudJoueurPartenaire;

		// Si le joueur est un homme, son partenaire est le suivant
		if (joueur.getSexe() == 'H') {
			noeudJoueurPartenaire = noeudJoueur.suivant;
			noeudJoueur.precedent.suivant = noeudJoueurPartenaire.suivant;
			noeudJoueurPartenaire.suivant.precedent = noeudJoueur.precedent;
			mapJoueurNoeud.remove(noeudJoueur.suivant.joueur);
		}
		// Si le joueur est une femme, son partenaire est le précédent
		else if (joueur.getSexe() == 'F') {
			noeudJoueurPartenaire = noeudJoueur.precedent;
			noeudJoueur.suivant.precedent = noeudJoueurPartenaire.precedent;
			noeudJoueurPartenaire.precedent.suivant = noeudJoueur.suivant;
			mapJoueurNoeud.remove(noeudJoueur.precedent.joueur);
		}
		mapJoueurNoeud.remove(noeudJoueur.joueur);
		return true;
	}
