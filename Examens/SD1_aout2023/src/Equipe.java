...
public class Equipe {
    ...
    public boolean ajouterEnFin(String nouvelEquipier){
        ...
        Noeud noeudNouvelEtudiant = new Noeud(nouvelEquipier);

        Noeud noeudAvant = queue.precedent;
        Noeud noeudApres = queue;

        noeudNouvelEtudiant.suivant = noeudApres;
        noeudNouvelEtudiant.precedent = noeudAvant;
        noeudApres.precedent = noeudNouvelEtudiant;
        noeudAvant.suivant = noeudNouvelEtudiant;

        mapEquipierNoeud.put(nouvelEquipier, noeudNouvelEtudiant);
        return true;
    }
    ...
    public boolean tronquerApres(String equipier){
        ...
        Noeud baladeur = mapEquipierNoeud.get(equipier);

        if (baladeur == queue.precedent)
            return false;

        Noeud suivant = baladeur.suivant;

        while (suivant != null) {
            mapEquipierNoeud.remove(suivant.equipier);
            if (suivant.suivant != null) {
                suivant.suivant.precedent = baladeur;
                baladeur.suivant = suivant.suivant;
            }
