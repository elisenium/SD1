...
public class ArbreDEntiers implements Iterable<Integer>{
	...
	private class IterateurParNiveau implements Iterator<Integer> {
		...
		public Integer next() {
			NoeudEntier noeudEntier = fileDeNoeuds.removeFirst();
			if (noeudEntier.gauche != null) {
				fileDeNoeuds.add(noeudEntier.gauche);
			}
			if (noeudEntier.droit != null) {
				fileDeNoeuds.add(noeudEntier.droit);
			}
			return noeudEntier.entier;
			//cette methode "defile"
		}
	}
}
