...
public class ClasseEtudiants{
	...
	public SerieEtudiants getSerieEtudiants(int numeroSerie) throws IllegalArgumentException{
		if (numeroSerie > tableSeries.length || numeroSerie < 1)
			throw new IllegalArgumentException();
	...
	public boolean changerSerie(String nom, int nouveauNumeroSerie){
		if (nom.equals("") || nom == null || nouveauNumeroSerie < tableSeries.length || nouveauNumeroSerie > tableSeries.length)
			throw new IllegalArgumentException();

		if (numeroSerie(nom) == nouveauNumeroSerie) return false;
