...
public class GestionDesSeries {
	...
	private static void rechercherSerie() {
		...
		if (numeroSerie == -1) {
			System.out.println("Cet(te) étudiant(e) n'a pas été retrouvé(e) dans une des séries");

		} else {
			System.out.println("La série de l'étudiant(e) est la série " + numeroSerie);
	...
	private static void afficherTailleSeries() {
		for (int i = 1; i <= classe.nombreSeries(); i++) {
			System.out.println("Série n°" + i + " : " + classe.getSerieEtudiants(i).nombreEtudiants() + " étudiants");
		}
	}
